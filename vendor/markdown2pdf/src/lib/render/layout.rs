...
struct Engine<'a> {
    ...
    in_code_block: bool,
    /// True while drawing table cell text. Line-level page breaks in
    /// `write_wrapped_runs` are suppressed so a row isn't split in half.
    in_table: bool,
...
impl<'a> Engine<'a> {
    ...
    fn remaining_pt(&self) -> f32 {
        (self.page_height_pt() - self.bottom_margin_pt() - self.y_from_top_pt).max(0.0)
    }

    fn page_body_pt(&self) -> f32 {
        (self.page_height_pt() - self.top_margin_pt() - self.bottom_margin_pt()).max(0.0)
    }

    /// Push the upcoming block to the next page when starting it here
    /// would leave a stub, or when the whole block fits on a page but
    /// not in the leftover space.
    fn avoid_split_start(&mut self, needed: f32, min_stub: f32) {
        if (self.y_from_top_pt - self.top_margin_pt()).abs() < 0.01 {
            return;
        }
        let rem = self.remaining_pt();
        let body = self.page_body_pt();
        if needed <= 0.0 {
            return;
        }
        if (needed <= body && needed > rem) || rem < min_stub.min(needed) {
            self.advance_column();
        }
    }
    ...
    fn render_table(
        ...
        let first_data_h = row_heights.first().copied().unwrap_or(0.0);
        let table_h = header_height
            + row_gap_pt
            + row_heights.iter().sum::<f32>()
            + row_gap_pt * table_rows.len() as f32
            + after_pt;
        let header_and_row = header_height + row_gap_pt + first_data_h;
        self.avoid_split_start(table_h, header_and_row);
        self.in_table = true;
        self.letter_spacing_pt = s_header.letter_spacing_pt;
    ...
    fn render_code_block(&mut self, lines: &[String]) {
        ...
        for line in lines {
            ...
            wrapped += self.estimate_wrapped_lines(std::slice::from_ref(&run), s.font_size_pt, base);
        }
        let content_h = line_h * wrapped.max(1) as f32;
        let needed = s.margin_before_pt + s.padding.top + s.padding.bottom + content_h;
        let stub = line_h * 3.0 + s.padding.top + s.padding.bottom;
        self.avoid_split_start(needed, stub);
        let ctx = self.begin_block(&s);
    ...
    fn write_wrapped_runs(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            // start.
            if !self.in_table
                && self.y_from_top_pt + line_height_pt + self.bottom_margin_pt()
                    > self.page_height_pt()
                && (self.y_from_top_pt - self.top_margin_pt()).abs() >= 0.01
            {
                self.close_text_section();
                self.advance_column();
            }
